...
//连接数据库
var connection_string = '127.0.0.1:27017/RestifyDemo';
var db = mongojs(connection_string, ['RestifyDemo']);
var jobs = db.collection("jobs");

//配置插件
server.use(restify.plugins.acceptParser(server.acceptable));
server.use(restify.plugins.queryParser());//解析HTTP查询字符串
server.use(restify.plugins.bodyParser());//请求数据转换为JavaScript对象

//编写CRUD API
var PATH = '/jobs';
server.get({path: PATH, version: '0.0.1'}, findAllJobs);
server.get({path: PATH + '/:jonId', version: '0.0.1'}, findJob);
